 ...
 d3.csv('amenities/external_util.csv', function(data) {
    data.forEach(function(item) {
        ...
        L.marker([latitude, longitude], {icon: pinIcon}).addTo(map)
            .bindPopup('Name: ' + item.name + '<br/> Type: ' + item.type + ' ' + item.power + '<br/> No. of Chargers: ' + item["# of chargers"] + '<br/> Peak Usage: ' + Number(item['peak utilisation']).toFixed(3) + '<br/> Hour that it occured: ' + item['when peak']) // Customize the popup content
    });
 });
